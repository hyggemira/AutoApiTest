...
def disconnect(conn):
    try:
        ...
        print(f"断开数据连接失败，异常信息为{e}")


# 执行sql语句
def execute(conn, sql): # 相当于建了一条到数据库的路
    try:
        cursor = conn.cursor()  # 获取游标，相当于在路上跑的一辆车，通过车把数据取回来
        cursor.execute(sql) # 执行sql语句
        # print(cursor.fetchall())  打印所有列表信息
        conn.commit() # 提交
        cursor.close()  # 关闭游标
...
if __name__ == '__main__':
    ...
    conn = connect(db)
    execute(conn, "delete from Member where MobilePhone=13112345628;")
    # execute(conn, "select * from Member;")
    # execute(conn, 'select * from Member where MobilePhone=13112345628')
